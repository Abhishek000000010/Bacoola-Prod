        ...
        <PopoverButton className="h-full focus:outline-none">
          ...
          >
            ...
            <div className="small:hidden relative flex items-center">
              {/* The icon's name for screen readers. It lives in this
                  phone-only block so desktop keeps its visible "Bag (n)". */}
              <span className="sr-only">{`Bag (${totalItems})`}</span>
              ...
              {totalItems > 0 && (
                <span
                  aria-hidden
                  className="absolute -top-1 -right-1 bg-red-600 text-white text-[12px] w-4 h-4 flex items-center justify-center rounded-full font-bold"
                >
                  {totalItems}
                </span>
              )}
